...
export class CheckoutPaymentComponent implements OnInit{
  ...
  ngOnInit(): void {
    loadStripe('pk_test_BdLwAbhnUrbZoFkACdRCq0D100jybT9Ad4').then( stripe => {
      this.stripe=stripe;
...
get paymentFormComplete()
{
        ...
        if(paymentResult.paymentIntent)
            {
              ...
              const navigationExtra : NavigationExtras = {state: createdOrder};
  ...
  private async confirmPaymentWithStripe(basket: IBasket) {
    ...
    if(!result) throw new Error('Problem attempting with stripe');
    return result;
  }
